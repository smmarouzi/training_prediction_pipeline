...
from src.prepare_data.utils import (
    calculate_euclidean_dist,
    calculate_haversine_dist,
    calculate_avg_distance_to_restaurants,
    calculate_avg_Hdist_to_restaurants,
)
...
def prepare_restaurants_ids(df: pd.DataFrame) -> dict[str, dict[str, float | int]]:
    ...
    #unique restaurants
    restaurants_ids: dict[str, dict[str, float | int]] = {}
    for a, b in zip(df.restaurant_lat, df.restaurant_lon):
        idx = f"{a}_{b}"
        restaurants_ids[idx] = {"lat": float(a), "lon": float(b)}
...
def add_restaurants_ids(df: pd.DataFrame) -> pd.DataFrame:
    """Add restaurant identifier column."""
    logger.info('ADDING RESURANT ID COLUMN...')
    
    restaurants_ids = prepare_restaurants_ids(df)
    df["restaurant_id"] = [
        restaurants_ids[f"{a}_{b}"]["id"] for a, b in zip(df.restaurant_lat, df.restaurant_lon)
    ]

    return df


def assign_centroids(df: pd.DataFrame) -> pd.DataFrame:
    """Assign each observation to a centroid using Euclidean distance."""
...
def add_distance_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Add multiple distance measurements between couriers and restaurants."""
    
    restaurants_ids = prepare_restaurants_ids(df)
    logger.info('ADDING 4 DISTANCE COLUMNS...')
    df["dist_to_restaurant"] = calculate_euclidean_dist(
    ...
    df["Hdist_to_restaurant"] = calculate_haversine_dist(
    ...
    df["avg_Hdist_to_restaurants"] = [
        calculate_avg_Hdist_to_restaurants(lat, lon, restaurants_ids)
        for lat, lon in zip(df.courier_lat, df.courier_lon)
    ]
...
def add_datetime_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Convert datetime columns and extract day and hour features."""
    
    logger.info('FIXING DATETIME FORMAT...')
    df["courier_location_timestamp"] = pd.to_datetime(
        df["courier_location_timestamp"], errors="coerce"
    )
    df["order_created_timestamp"] = pd.to_datetime(
        df["order_created_timestamp"], errors="coerce"
    )
...
def add_h3_clustering(df: pd.DataFrame) -> pd.DataFrame:
    """Add H3 geospatial clustering index."""
    logger.info('ADDING H3 CLUSTERING...')    
    
    df["h3_index"] = [
        h3.geo_to_h3(lat, lon, H3.resolution)
        for (lat, lon) in zip(df.courier_lat, df.courier_lon)
    ]
    return df


def add_orders_busyness(df: pd.DataFrame) -> pd.DataFrame:
    """Add orders busyness aggregated by H3 and hour."""
...
def add_restaurants_per_index(df: pd.DataFrame) -> pd.DataFrame:
    """Add the count of restaurants per H3 index."""
...
def convert_to_correct_dtype(df: pd.DataFrame) -> pd.DataFrame:
    """Correct the dtype of the ``h3_index`` column."""
...
def encoder(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for feature in columnsToEncode:
        ...
        except Exception:  # pragma: no cover - best-effort encoding
...
def data_preprocessing_pipeline(df: pd.DataFrame) -> pd.DataFrame:
    """Run the full data preprocessing pipeline on ``df``."""
